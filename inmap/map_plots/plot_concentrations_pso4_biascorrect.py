...
# Load the shapefile
shapefile_path = "../inputs/PSO4_regridded.shp"
gdf = gpd.read_file(shapefile_path)

# Extract "MortalityRa" values and apply bias correction
#bias_correction_factor = 2.1654160880007356 #linear not dust
bias_correction_factor = 3.319161231867947 #quantile
...
aermass_bc = dataset.variables["AerMassSO4"][0, 0, :, :]
...
gdf.plot(column="MortalityRa_corrected", cmap="RdYlGn", vmin=vmin, vmax=vmax, legend=True,
         ax=axes[0], legend_kwds={"label": "pSO4 concentrations (ug/m3)"})
...
im = axes[1].pcolormesh(lon_mesh, lat_mesh, aermass_bc, cmap="RdYlGn", shading="auto",
                        vmin=vmin, vmax=vmax)
fig.colorbar(im, ax=axes[1], label="pSO4 concentrations (ug/m3)")
...
# Show the plot
plt.savefig("../outputs/maps/plot_pso4.png")
