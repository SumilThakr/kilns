...
# Load the shapefile
shapefile_path = "../inputs/PrimPM_regridded.shp"
gdf = gpd.read_file(shapefile_path)

# Extract "MortalityRa" values and apply bias correction
#bias_correction_factor = 0.23420725542578988 # linear
bias_correction_factor = 0.12778081510958195 # quantile
...
aermass_bc = dataset.variables["PM25"][0, 0, :, :] - (dataset.variables["AerMassSAL"][0, 0, :, :] * 1.86) - 1.1 * (dataset.variables["AerMassNIT"][0, 0, :, :] + dataset.variables["AerMassSO4"][0, 0, :, :] + dataset.variables["AerMassNH4"][0, 0, :, :]) - 1.05*(dataset.variables["TotalOA"][0, 0, :, :] - dataset.variables["AerMassPOA"][0, 0, :, :])
...
# Plot the bias-corrected shapefile data
...
gdf.plot(column="MortalityRa_corrected", cmap="RdYlGn", vmin=vmin, vmax=vmax, legend=True,
         ax=axes[0], legend_kwds={"label": "Primary PM2.5 concentrations (ug/m3)"})
...
im = axes[1].pcolormesh(lon_mesh, lat_mesh, aermass_bc, cmap="RdYlGn", shading="auto",
                        vmin=vmin, vmax=vmax)
fig.colorbar(im, ax=axes[1], label="Primary PM2.5 concentrations (ug/m3)")
...
# Show the plot
plt.savefig("../outputs/maps/plot_prim.png")
