...
# Load the shapefile
shapefile_path = "../inputs/SOA_regridded.shp"
gdf = gpd.read_file(shapefile_path)

# Extract "MortalityRa" values and apply bias correction
bias_correction_factor = 1.0 #0.0024043967236390086
...
aermass_bc = dataset.variables["TotalOA"][0, 0, :, :] - dataset.variables["AerMassPOA"][0, 0, :, :]
...
gdf.plot(column="MortalityRa_corrected", cmap="RdYlGn", vmin=vmin, vmax=vmax, legend=True,
         ax=axes[0], legend_kwds={"label": "SOA concentrations (ug/m3)"})
...
im = axes[1].pcolormesh(lon_mesh, lat_mesh, aermass_bc, cmap="RdYlGn", shading="auto",
                        vmin=vmin, vmax=vmax)
fig.colorbar(im, ax=axes[1], label="SOA concentrations (ug/m3)")
...
# Show the plot
plt.savefig("../outputs/maps/plot_soa.png")
