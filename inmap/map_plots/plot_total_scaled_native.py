...
shapefile_info = {
    "../inputs/PNO3_native.shp":   0.1143236952329479 / (1.1 * (1.0 + 18.04/62.0049)), # The 1.1 refers to the scaling in the GC PM2.5 eqn, and the other factor accounts for pNH4
    "../inputs/PrimPM_native.shp": 0.12778081510958195,
    "../inputs/PSO4_native.shp":   3.319161231867947 / (1.1 * (1.0 + 2 * 18.04/96.06)),
    "../inputs/SOA_native.shp":    1.0 / 1.05, # The 1.05 refers to the scaling in the GC PM2.5 eqn
}
...
output_shapefile = "../intermediate/total_PM25_native.shp"
gdf_template.to_file(output_shapefile)

# Load the NetCDF file
netcdf_path = "$PROJECT/GC/concentrations/2025/dst/diff_dst_clipped.nc4"
...
gdf_template.plot(column="Total_PM25_Corrected", cmap="RdYlGn", vmin=vmin, vmax=vmax, legend=True,
                  ax=axes[0], legend_kwds={"label": "PM2.5"})
axes[0].set_title("Total PM2.5")
axes[0].set_xticks([])
axes[0].set_yticks([])
...
# Show the plot
plt.savefig("../outputs/maps/total_scaled_native.png")
